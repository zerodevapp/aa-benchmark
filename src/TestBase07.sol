...
import {
    ENTRYPOINT_SIMULATION_0_7_INITCODE, ENTRYPOINT_SIMULATION_0_7_ADDR
} from "src/artifacts/EntrypointArtifacts.sol";
...
abstract contract AAGasProfileBase07 is Test {
    ...
    function setUp() public virtual {
        ...
        entryPoint = IEntryPoint(
            ArtifactsLib.deploy(DeployArtifact({initCode: ENTRYPOINT_0_7_INITCODE, addr: ENTRYPOINT_0_7_ADDR}))
        );

        simulation = IEntryPointSimulations(
            ArtifactsLib.deploy(
                DeployArtifact({initCode: ENTRYPOINT_SIMULATION_0_7_INITCODE, addr: ENTRYPOINT_SIMULATION_0_7_ADDR})
            )
        );
    ...
    function calculateUserOpGasLimits(PackedUserOperation memory _op) internal returns (bytes32 res) {
        uint128 validationGasLimit = 60000; // offset
        try this.getVerificationGasLimit(_op) {}
        ...
        try this.getExecutionGasLimit(_op.callData) {}
        catch (bytes memory reason) {
            require(reason.length == 36, "Not proper error");
    ...
    function testTransferERC20(address _recipient, uint256 _amount) public {
        ...
        PackedUserOperation memory op = fillUserOp(
            _fillData(address(mockERC20), 0, abi.encodeWithSelector(mockERC20.transfer.selector, _recipient, _amount))
        );
