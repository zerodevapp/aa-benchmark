...
interface SmartAccountFactory {
    function deployCounterFactualAccount(address _owner, uint256 _index) external returns (address proxy);
    function getAddressForCounterFactualAccount(address _owner, uint256 _index)
        external
        view
...
interface SmartAccount {
    function executeCall(address _to, uint256 _value, bytes calldata _data) external;
}
