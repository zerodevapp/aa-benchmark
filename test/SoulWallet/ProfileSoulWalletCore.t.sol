...
import {SOULWALLETCORE_IMPL, SOULWALLETCORE_IMPL_BYTECODE, SOULWALLETCORE_FACTORY, SOULWALLETCORE_FACTORY_BYTECODE, SoulWalletFactory, SoulWalletCore} from "./SoulWalletCoreArtifacts.sol";
...
contract ProfileBcnmy is AAGasProfileBase {
    ...
    function fillData(
        ...
        return
            abi.encodeWithSelector(
                SoulWalletCore.execute.selector,
                _to,
                _value,
                _data
            );
    ...
    function getSignature(
        ...
        bytes32 hash = ECDSA.toEthSignedMessageHash(
            _packHash(_op.sender, userOpHash)
        );
    ...
    function createAccount(address _owner) internal override {
        ...
        bytes memory initializer = abi.encodeWithSelector(
            SoulWalletCore.initialize.selector,
            ownerBytes32
        );
    ...
    function getAccountAddr(
        ...
        (bool success, bytes memory data) = address(factory).staticcall(
            abi.encodeWithSelector(
                factory.getWalletAddress.selector,
                initializer,
                0
            )
        );
    ...
    function getInitCode(
        ...
        return
            abi.encodePacked(
                ...
                abi.encodeWithSelector(
                    SoulWalletFactory.createWallet.selector,
                    initializer,
                    0
                )
            );
