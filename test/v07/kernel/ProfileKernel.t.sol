...
contract ProfileKernel is AAGasProfileBase07 {
    ...
    function _initializeTest() internal override {
        ...
        address kernel = ArtifactsLib.deploy(DeployArtifact({
            initCode: KERNEL_3_3_INITCODE,
            addr: KERNEL_3_3_ADDR
        }));

        kernelFactory = IKernelFactory(ArtifactsLib.deploy(DeployArtifact({
            initCode: KERNEL_3_3_FACTORY_INITCODE,
            addr: KERNEL_3_3_FACTORY_ADDR
        })));

        ecdsaValidator = ArtifactsLib.deploy(DeployArtifact({
            initCode: ECDSA_VALIDATOR_INITCODE,
            addr: ECDSA_VALIDATOR_ADDR
        }));
    }
    ...
    function _fillData(address _recipient, uint256 _amount, bytes memory _data) internal override returns (bytes memory) {
        return abi.encodeWithSelector(
            IKernel.execute.selector,
            bytes32(0),
            abi.encodePacked(_recipient, _amount, _data)
        );
    }
    ...
    function _getInitCode() internal override returns (bytes memory) {
        return abi.encodePacked(
            ...
            abi.encodeWithSelector(
                IKernelFactory.createAccount.selector,
                initializeData(),
                bytes32(0)
            )
        );
